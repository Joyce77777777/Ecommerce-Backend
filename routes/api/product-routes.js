...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      handleError(res, 404, "Product with the specified ID not found");
      return;
    }

    res.status(200).json(product);
  } catch (err) {
    handleError(res, 500, "Failed to fetch the product");
  }
});
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    handleError(res, 500, "Failed to delete the product");
  }
});
